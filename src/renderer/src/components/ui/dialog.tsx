...
interface HistoryDialogProps extends DialogPrimitive.DialogProps {
  ...
  disableOutsideClickToClose?: boolean; // 添加新属性
}

const DialogRoot = ({
  enableHistoryBack = false,
  disableOutsideClickToClose = false, // 默认不禁用点击外部关闭
  open,
  onOpenChange,
  ...props
}: HistoryDialogProps) => {
  ...
  const handleOpenChange = (open: boolean) => {
    // 如果是关闭操作且禁用了外部关闭（点击外部或按ESC），则不执行关闭
    if (!open && disableOutsideClickToClose) {
      return;
    }
    setInternalOpen(open);
    if (onOpenChange) {
...
>(({ className, disableOutsideClickToClose, ...props }, ref) => {
  // 如果禁用点击外部关闭，则不执行默认的关闭行为
...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & {
    disableOutsideClickToClose?: boolean;
    hideCloseButton?: boolean; // 添加控制关闭按钮显示的属性
  }
>(({ className, children, disableOutsideClickToClose, hideCloseButton = false, ...props }, ref) => (
